...
ttf_path = os.path.abspath(os.path.dirname(__file__))
...
class Uploader:
	...
	def __init__(self, fileobj, config, static_folder, _type=None,method=0):
		"""
	...
	def upBase64(self):
		...
		try:
			...
			if self.method==0:
				self.image_watermark(self.filePath)
		except:
	...
	def upFile(self):
		...
		if self.method == 0:
			# self.image_watermark(self.filePath)
			self.qiniuurl = qn.upload(qiniu_name=self.filePath.split("/")[-1],local_file=self.filePath)
	...
	def getQNfileInfo(self):
		filename = self.qiniuurl
		print filename
		return {
			'state': self.stateInfo,
			'url': filename,
	...
	def image_watermark(self,path):
		"""  图片压缩  """
		image = Image.open(path).convert('RGB')
		orign_w, orign_h = image.size
		rate = 1.0
		if orign_w > 800:
			rate = 800.0 / orign_w
		dest_w, dest_h = int(orign_w * rate), int(orign_h * rate)
		image.thumbnail((dest_w, dest_h), Image.ANTIALIAS)
		image.save(path, "JPEG")
		"""图片添加水印"""
		im = Image.open(path).convert('RGBA')
		txt=Image.new('RGBA', im.size, (0,0,0,0))
		fnt=ImageFont.truetype(ttf_path+"/华文细黑.TTF", 20)
		d=ImageDraw.Draw(txt)
		d.text((txt.size[0]/2-30-1,txt.size[1]-30),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30+1,txt.size[1]-30),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30,txt.size[1]-30-1),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30,txt.size[1]-30+1),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')

		d.text((txt.size[0]/2-30-1,txt.size[1]-30-1),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30+1,txt.size[1]-30-1),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30-1,txt.size[1]-30+1),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30+1,txt.size[1]-30+1),  unicode("@好投顾",'UTF-8'),font=fnt, fill='gray')
		d.text((txt.size[0]/2-30,txt.size[1]-30),  unicode("@好投顾",'UTF-8'),font=fnt, fill=(255,255,255,255))
